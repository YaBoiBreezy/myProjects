    ...
    for event in events:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if keys[pygame.K_LEFT]:
                game_data=handle_key_left(game_data)
            if keys[pygame.K_RIGHT]:
                game_data=handle_key_right(game_data)
            if keys[pygame.K_ESCAPE]:
                game_data=handle_key_escape(game_data)
...
def update_paddle(entity):
    ...
    if entity['current_action'] == 'NA':
        return
    if entity['current_action']=='left' and entity['location'][0]>0:
        entity['location'][0]-=entity['velocity']
    if entity['current_action']=='right' and entity['location'][0]<720:
        entity['location'][0]+=entity['velocity']
...
def render_raw_data(game_data):
    for entity in game_data['entities']:
        if entity['type']=='ball':
            print('ball:({});'.format(entity['location']), end=' ')
    print(' ')
