...
function sendSingleProduct(req, res, next){
	res.format({
		...
		"text/html": () => {
			let prodcts = req.session.prodcts || [];
			if(!prodcts.includes(req.params.id))
				prodcts.push(req.params.id);
			req.session.prodcts = prodcts;

			let recommended=[]
            for(let x=0;x<5 && x<prodcts.length;x++){
                recommended.push(prodcts[x]);
            }
            while(recommended.length<5){
                let x=Math.random().toString().substr(2,3);
                if(req.app.locals.products[x]!=undefined){
                    recommended.push(x);
                }
            }

			res.render("pages/product", {product: req.product, recommend: recommended});
		}
	});
	next();
}
